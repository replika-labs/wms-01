...
const { MaterialNew, PurchaseLog, MaterialMovement } = require('../models');
...
const getMaterials = asyncHandler(async (req, res) => {
  ...
  const { count, rows: materials } = await MaterialNew.findAndCountAll({
    where,
...
const getMaterialById = asyncHandler(async (req, res) => {
  const material = await MaterialNew.findByPk(req.params.id);
...
const createMaterial = asyncHandler(async (req, res) => {
  ...
  if (code) {
    const existingMaterial = await MaterialNew.findOne({ where: { code } });
    if (existingMaterial) {
      res.status(400);
  ...
  // Create material
  const material = await MaterialNew.create({
    name,
    code,
    fabricTypeColor,
    purchaseDate,
