  ...
  return (
    <div className="bg-gray-800 border border-gray-600 rounded-lg p-4 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">
            ...
            {alreadyCompleted > 0 && (
              <span className="text-xs text-blue-400 ml-1">(Additional to {alreadyCompleted} already done)</span>
            )}
          </label>
          <input
            type="number"
            ...
            className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-700 text-white ${errors[`product_${product.id}_pcs`] ? 'border-red-500' : 'border-gray-600'
              } ${remainingToComplete === 0 ? 'bg-gray-600' : ''}`}
          ...
          {errors[`product_${product.id}_pcs`] && (
            <p className="text-xs text-red-400 mt-1">{errors[`product_${product.id}_pcs`]}</p>
          )}
          <div className="text-xs text-gray-400 mt-1">
            {remainingToComplete > 0 ? (
              ...
              <p className="text-green-400">✅ This product is fully completed</p>
            )}
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">
            ...
            {product.baseMaterial && (
              <span className="text-xs text-gray-400 ml-1">({product.baseMaterial.unit})</span>
            )}
          </label>
          <div className="flex">
            <input
            ...
            <button
              ...
              className="px-3 py-2 bg-blue-600 text-white text-xs rounded-r-md hover:bg-blue-700 transition-colors"
      ...
      <div className="mb-4">
        ...
        {/* Photo Upload Area */}
        <div
          className={`border-2 border-dashed rounded-lg p-4 text-center transition-colors ${isDragOver ? 'border-blue-400 bg-blue-900/20' : 'border-gray-600 hover:border-gray-500'
            }`}
            ...
            {localProgress.photos.map((photo, index) => (
              <div key={photo.id || index} className="relative group">
                ...
                <input
                  ...
                  className="mt-1 w-full px-2 py-1 text-xs border border-gray-600 rounded bg-gray-700 text-white"
                  disabled={isSubmitting}
                />
              </div>
            ))}
      ...
      {(localProgress.pcsFinished > 0 || alreadyCompleted > 0) && (
        <div className="bg-gray-700 rounded-lg p-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            ...
            <div>
              <div className="text-gray-400">This Session</div>
              <div className="font-medium text-white">
                +{localProgress.pcsFinished} pieces
              </div>
            </div>
            <div>
              <div className="text-gray-400">Material/Piece</div>
              <div className="font-medium text-white">
